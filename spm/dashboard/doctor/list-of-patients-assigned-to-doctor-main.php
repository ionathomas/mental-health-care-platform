<?php
    ...
    if(isset($_SESSION['UserName'])){
        ...
        //$sql = "SELECT * FROM assignpatient WHERE AdminUserName = '$UserName'" ;
        $sql = "SELECT * FROM assignpatient WHERE AdminUserName = '$UserName' AND PatientUserName NOT IN (SELECT PatientUserName FROM appointment WHERE MgtCategory = 'D' UNION SELECT PatientUserName FROM deletepatient UNION SELECT PatientUserName FROM pending_appointment);" ;
        ...
        if($len > 0){
          ...
          while($row = mysqli_fetch_assoc($result)){
            ...
            if(strcmp($row['AssignerCategory'],"C")==0){
              $sql4 = "SELECT FullName FROM counselor WHERE UserName = '$ab' ";
              $result4 = mysqli_query($conn, $sql4);
              $row4 = mysqli_fetch_assoc($result4);
              $assignedBy[$i] = $row4['FullName'];
            }
            else{
              $sql5 = "SELECT FullName FROM manager WHERE UserName = '$ab' ";
              $result5 = mysqli_query($conn, $sql5);
              $row5 = mysqli_fetch_assoc($result5);
              $assignedBy[$i] =  $row5['FullName']; 
            }
          ...
          if(isset($_POST['patientUserName'])){
            ...
            if(mysqli_query($conn,$sql6)){
              echo "<script>alert('Patient Deleted'); window.open('list-of-patients-assigned-to-doctor-main.php','_self');</script>";
              exit();
            }
            else{
              echo "<script>alert('Error Encountered while deleting Patient. Try again later'); window.open('list-of-patients-assigned-to-doctor-main.php','_self');</script>";
              exit();
            }
          }
        }
              ...
              <div class="mdc-layout-grid__cell stretch-card mdc-layout-grid__cell--span-12">
                <div class="mdc-card p-0">
                  <h6 class="card-title card-padding pb-2"
                    style="font-weight: 600; margin-bottom: 20px; font-size: 22px;">Assigned Patient</h6>
                  <div class="table-responsive">
                    <div class="row" style="display: flex; justify-content: center;">
                      <div class="col-md-12">
                      <h7 id="msg" style="visibility: visible; color: red">Currently no patient has been assigned...</h7>
  ...
  <script>
    ...
    function displayList(){
      ...
      if(len > 0){
        ...
        assignedBy=<?php echo '["' . implode('", "', $assignedBy) . '"]' ?>;
        ...
        for(i=0;i<len;i++){
          ...
          str+='<td style="width: 15%;">'+assignedBy[i]+'</td>'
          str+='<td style="width: 12%;">'+score[i]+'</td>'
          ...
          str+='<form id="deletePatient'+i+'" method="post" action="list-of-patients-assigned-to-doctor-main.php">'
