<?php
    ...
    if(isset($_SESSION['UserName'])){
      ...
      //CHANGE ASSIGNPATIENT2 TO ASSIGNPATIENT and APPOINTMENT2 TO APPOINTMENT
      $sql = "SELECT * FROM assignpatient WHERE AdminUserName = '$UserName' AND PatientUserName NOT IN (SELECT PatientUserName FROM assignpatient WHERE AdminCategory = 'D' UNION SELECT PatientUserName FROM appointment UNION SELECT PatientUserName FROM deletepatient UNION SELECT PatientUserName FROM pending_appointment);" ;
      ...
      if($len > 0){
        ...
        if(isset($_POST['patientUserName'])){
          ...
          if(mysqli_query($conn,$sql6)){
            ...
            echo "<script>alert('Error Encountered while deleting Patient. Try again later'); window.open('list-of-patients-assigned-to-counselor-main.php','_self');</script>";
            exit();
          }
        }
      }
              ...
              <div class="mdc-menu mdc-menu-surface" tabindex="-1">
                <ul class="mdc-list" role="menu" aria-hidden="true" aria-orientation="vertical">
                  <li class="mdc-list-item" role="menuitem">
                    ...
                    <div class="item-content d-flex align-items-start flex-column justify-content-center">
                      <h6 class="item-subject font-weight-normal">L<a href="../../logout.php">Logout</a></h6>
                    </div>
                  </li>
                </ul>
              </div>
    ...
    function displayList(){
      ...
      if(len > 0){
        ...
        for(i=0;i<len;i++){
          ...
          str+='<form id="deletePatient'+i+'" method="post" action="list-of-patients-assigned-to-counselor-main.php">'
